...
bool status_check_skilluse(struct block_list *src, struct block_list *target, uint16 skill_id, int flag)
{
	...
	if (tsc && tsc->count)
	{
		...
		if (!skill_id && tsc->data[SC_TRICKDEAD])
			return false;
		if ((skill_id == WZ_STORMGUST || skill_id == WZ_FROSTNOVA || skill_id == NJ_HYOUSYOURAKU || skill_id == NPC_STORMGUST2) && tsc->data[SC_FREEZE])
			return false;
...
void status_calc_misc(struct block_list *bl, struct status_data *status, int level)
{
	...
	else
	{
		...
		// Flee
		stat = status->flee;
		stat += level + status->agi + (bl->type == BL_MER ? 0 : bl->type == BL_PC ? status->luk / 5 + (status->agi / 10)
																																							: 0) +
...
void status_calc_regen_rate(struct block_list *bl, struct regen_data *regen, struct status_change *sc)
{
	...
	// No HP or SP regen
	if ((sc->data[SC_POISON] && !sc->data[SC_SLOWPOISON]) || (sc->data[SC_DPOISON] && !sc->data[SC_SLOWPOISON]) || sc->data[SC_TRICKDEAD] || sc->data[SC_BLEEDING] || (sc->data[SC_MAGICMUSHROOM] && sc->data[SC_MAGICMUSHROOM]->val3 == 1) || sc->data[SC_SATURDAYNIGHTFEVER] || sc->data[SC_REBOUND])
		regen->flag = RGN_NONE;
...
void status_calc_bl_main(struct block_list *bl, /*enum scb_flag*/ int flag)
{
	...
	if (flag & SCB_ASPD)
	{
		...
		else if (bl->type & BL_PC)
		{
			...
			maxaspd = cap_value(maxaspd, 50, 2000);
			sc = status_get_sc(bl);
...
int status_change_start(struct block_list *src, struct block_list *bl, enum sc_type type, int rate, int val1, int val2, int val3, int val4, t_tick duration, unsigned char flag)
{
	...
	case SC_MODECHANGE:
	{
		...
		if (val4)
			mode = (mode & ~val4); // Del mode
		if (val3)
			mode = (mode | val3); // Add mode
	...
	if (!(flag & SCSTART_LOADED)) // &4 - Do not parse val settings when loading SCs
		...
		case SC_BERSERK:
			...
			if (!val4)
				val4 = 10 + status->vit * 3; // Val4 holds damage interval
		...
		case SC_MARIONETTE:
		{
			...
			val3 = 0;
			val4 = 0;
			stat = ((sd ? sd->status.str : status_get_base_status(bl)->str) * (skill_lv * 10)) / 100;
			val3 |= cap_value(stat, 0, 0xFF) << 16;
			stat = ((sd ? sd->status.agi : status_get_base_status(bl)->agi) * (skill_lv * 10)) / 100;
			val3 |= cap_value(stat, 0, 0xFF) << 8;
			stat = ((sd ? sd->status.int_ : status_get_base_status(bl)->int_) * (skill_lv * 10)) / 100;
			val4 |= cap_value(stat, 0, 0xFF) << 16;
			stat = ((sd ? sd->status.dex : status_get_base_status(bl)->dex) * (skill_lv * 10)) / 100;
			val4 |= cap_value(stat, 0, 0xFF) << 8;
			stat = ((sd ? sd->status.luk : status_get_base_status(bl)->luk) * (skill_lv * 10)) / 100;
