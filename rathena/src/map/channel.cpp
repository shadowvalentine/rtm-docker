...
int channel_ajoin(struct guild *g)
{
	...
	for (i = 0; i < MAX_GUILDALLIANCE; i++)
	{
		...
		{
			for (j = 0; j < ag->max_member; j++)
			{ // load all guildmember
...
int channel_clean(struct Channel *channel, struct map_session_data *sd, int flag)
{
	...
	if (i < sd->channel_count)
	{
		...
		sd->channels[i] = NULL;
		sd->channel_tick[i] = 0;
		for (; i < sd->channel_count; i++)
		{ // slice move list down
...
int channel_pcquit(struct map_session_data *sd, int type)
{
	...
	if (type & (1 | 2) && channel_config.ally_tmpl.name[0] && sd->guild)
	{ // quit guild and ally chan
	...
	if (type & 4 && channel_config.map_tmpl.name[0] && channel_haspc(mapdata->channel, sd) == 1)
	{ // quit map chan
		channel_clean(mapdata->channel, sd, 0);
	}
	if (type & 8 && sd->channel_count)
	{ // quit all chan
		uint8 count = sd->channel_count;
		for (i = count - 1; i >= 0; i--)
		{ // going backward to avoid shifting
...
int channel_display_list(struct map_session_data *sd, const char *options)
{
	...
	else if (options[0] != '\0' && strcmpi(options, "mine") == 0)
	{																									// display chan I'm into
...
int channel_pcban(struct map_session_data *sd, char *chname, char *pname, int flag)
{
	...
	case 3:
	{
		...
		clif_displaymessage(sd->fd, output);
		for (cbe = (struct chan_banentry *)dbi_first(iter); dbi_exists(iter); cbe = (struct chan_banentry *)dbi_next(iter))
		{ // for all users
			if (cbe->char_name[0])
